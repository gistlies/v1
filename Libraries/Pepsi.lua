                ...
                if getsynasset then
                    ...
                    elseif (#image > 14) and (string.sub(image, 1, 14) == "synasseturl://") then
                        local x, e = pcall(function()
                            ...
                            local fold = isfolder("./Adroitlyz Hub")
                            if fold then
                            else
                                makefolder("./Adroitlyz Hub")
                            end
                            fold = isfolder("./Adroitlyz Hub/Themes")
                            if fold then
                            else
                                makefolder("./Adroitlyz Hub/Themes")
                            end
                            fold = isfolder("./Adroitlyz Hub/Themes/SynapseAssetsCache")
                            if fold then
                            else
                                makefolder("./Adroitlyz Hub/Themes/SynapseAssetsCache")
                            end
                            if not fold or not isfile("./Adroitlyz Hub/Themes/SynapseAssetsCache/" .. codename .. ".dat") then
                                local res = game:HttpGet(string.sub(image, 15))
                                if res ~= nil then
                                    writefile("./Adroitlyz Hub/Themes/SynapseAssetsCache/" .. codename .. ".dat", res)
                                end
                            end
                            return getsynasset(readfile("./Adroitlyz Hub/Themes/SynapseAssetsCache/" .. codename .. ".dat"))
                        end)
                        if x and e ~= nil then
                            return e
                        end
...
function library:CreateWindow(options, ...)
    ...
    if windowName and #windowName > 0 and library.WorkspaceName == "Adroitlyz Hub" then
        library.WorkspaceName = convertfilename(windowName, "Adroitlyz Hub")
    end
    local Library = Instance_new("ScreenGui")
    library.MainScreenGui, MainScreenGui = Library, Library
    ...
    Library.Name = "     "
    Library.Parent = library.gui_parent
    Library.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    Library.DisplayOrder = 10
    ...
    function windowFunctions:CreateTab(options, ...)
        options = (options and (type(options) == "string") and resolvevararg("Tab", options, ...)) or options or {
            Name = "Adroitlyz Style: Elite Lego Hax"
        }
        ...
        function tabFunctions:CreateSection(options, ...)
            ...
            if isfolder and makefolder and listfiles and readfile and writefile then
                function sectionFunctions:AddPersistence(options, ...)
                    ...
                    local realDropdownHolderList = Instance_new("UIListLayout")
                    local dropdownEnabled = false
                    if not isfolder("./Adroitlyz Hub") then
                        makefolder("./Adroitlyz Hub")
                    end
                    local common_string = "./Adroitlyz Hub/" .. tostring(custom_workspace or library.WorkspaceName)
                    local function resolvelist(nofold)
                        if custom_workspace ~= options.Workspace then
                            custom_workspace = options.Workspace
                            common_string = "./Adroitlyz Hub/" .. tostring(custom_workspace or library.WorkspaceName)
                        end
    ...
    function windowFunctions:CreateDesigner(options, ...)
        ...
        local daaata = {{"AddTextbox", "__Designer.Textbox.ImageAssetID", backgroundsection, {
            ...
            Workspace = "Adroitlyz Hub Themes",
            Flags = flags,
            Suffix = "Theme",
            Desginer = true
